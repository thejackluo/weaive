...
-- Index for identity document lookup (used in _fetch_identity)
CREATE INDEX IF NOT EXISTS idx_identity_docs_user_type
  ON identity_docs(user_id, type)
  WHERE deleted_at IS NULL;
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.columns
        ...
        RAISE NOTICE '✅ ai_runs.context_used column created';
    END IF;

    IF EXISTS (SELECT 1 FROM information_schema.columns
               WHERE table_name = 'ai_runs' AND column_name = 'context_assembly_time_ms') THEN
        RAISE NOTICE '✅ ai_runs.context_assembly_time_ms column created';
    END IF;

    IF EXISTS (SELECT 1 FROM information_schema.columns
               WHERE table_name = 'ai_runs' AND column_name = 'quality_flag') THEN
        RAISE NOTICE '✅ ai_runs.quality_flag column created';
    END IF;

    -- Check user_profiles columns
    IF EXISTS (SELECT 1 FROM information_schema.columns
               WHERE table_name = 'user_profiles' AND column_name = 'active_personality') THEN
        RAISE NOTICE '✅ user_profiles.active_personality column created';
    END IF;

    -- Check indexes
    IF EXISTS (SELECT 1 FROM pg_indexes
               WHERE indexname = 'idx_subtask_completions_user_recent') THEN
        RAISE NOTICE '✅ idx_subtask_completions_user_recent index created';
    END IF;

    IF EXISTS (SELECT 1 FROM pg_indexes
               WHERE indexname = 'idx_journal_entries_user_recent') THEN
        RAISE NOTICE '✅ idx_journal_entries_user_recent index created';
    END IF;

    IF EXISTS (SELECT 1 FROM pg_indexes
               WHERE indexname = 'idx_goals_user_status_recent') THEN
        RAISE NOTICE '✅ idx_goals_user_status_recent index created';
    END IF;

    IF EXISTS (SELECT 1 FROM pg_indexes
               WHERE indexname = 'idx_identity_docs_user_type') THEN
        RAISE NOTICE '✅ idx_identity_docs_user_type index created';
    END IF;
END $$;
