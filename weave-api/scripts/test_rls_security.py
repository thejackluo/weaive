...
class RLSPenetrationTester:
        ...
        for user in self.test_users:
            try:
                ...
                for journal_idx in range(3):
                    journal_data = {
                        "user_id": user["profile_id"],
                        "local_date": (datetime.now() - timedelta(days=journal_idx)).strftime(
                            "%Y-%m-%d"
                        ),
    ...
    def test_direct_id_queries(self):
        ...
        for attacker in self.test_users[:3]:  # First 3 users attack
            ...
            for victim in self.test_users:
                ...
                try:
                    response = (
                        attacker_client.table("goals")
                        .select("*")
                        .eq("user_id", victim["profile_id"])
                        .execute()
                    )
                    ...
                    if response.data and len(response.data) > 0:
                        ...
                        logger.error(
                            f"  🚨 VULNERABILITY: User {attacker['index']} accessed User {victim['index']} goals!"
                        )
                    else:
                        logger.debug(
                            f"  ✅ User {attacker['index']} blocked from User {victim['index']} goals"
                        )
    ...
    def test_enumeration_attacks(self):
        ...
        for attacker in self.test_users[:3]:
            ...
            for table in tables:
                ...
                try:
                    ...
                    if response.data:
                        unique_users = set(
                            row.get("user_id") for row in response.data if "user_id" in row
                        )
                        if len(unique_users) > 1 or (
                            len(unique_users) == 1
                            and list(unique_users)[0] != attacker["profile_id"]
                        ):
                            self.attack_attempts["successful_attacks"] += 1
                            self.vulnerabilities.append(
                                {
                            ...
                            logger.error(
                                f"  🚨 VULNERABILITY: User {attacker['index']} saw multiple users' data in {table}!"
                            )
        ...
        logger.info(
            f"  Enumeration attacks: {self.attack_attempts['enumeration_attacks']} attempts"
        )
    ...
    def test_cross_user_inserts(self):
        ...
        try:
            ...
            if response.data and len(response.data) > 0:
                ...
                logger.error(
                    f"  🚨 VULNERABILITY: User {attacker['index']} inserted data for User {victim['index']}!"
                )
            else:
    ...
    def test_cross_user_updates(self):
        ...
        try:
            ...
            if response.data and len(response.data) > 0:
                ...
                logger.error(
                    f"  🚨 VULNERABILITY: User {attacker['index']} updated User {victim['index']} data!"
                )
            else:
    ...
    def test_cross_user_deletes(self):
        ...
        try:
            ...
            victim_journals = (
                self.supabase.table("journal_entries")
                .select("id")
                .eq("user_id", victim["profile_id"])
                .limit(1)
                .execute()
            ...
            _ = (
                attacker_client.table("journal_entries")
                .delete()
                .eq("id", victim_journal_id)
                .execute()
            )
            ...
            if not check.data:
                ...
                logger.error(
                    f"  🚨 VULNERABILITY: User {attacker['index']} deleted User {victim['index']} data!"
                )
            else:
    ...
    def test_immutability_violations(self):
        ...
        try:
            ...
            # Create template
            template_response = (
                user_client.table("subtask_templates")
                .insert(
                    {
                        "user_id": user["profile_id"],
            ...
            if not template_response.data:
                logger.warning(
                    "  ⚠️  Could not create subtask template (skipping immutability test)"
                )
                return
            ...
            # Create instance
            instance_response = (
                user_client.table("subtask_instances")
                .insert(
                    {
                        "user_id": user["profile_id"],
            ...
            if not instance_response.data:
                logger.warning(
                    "  ⚠️  Could not create subtask instance (skipping immutability test)"
                )
                return
            ...
            # Create completion
            completion_response = (
                user_client.table("subtask_completions")
                .insert(
                    {
                        "user_id": user["profile_id"],
            ...
            try:
                ...
                if check.data and check.data[0]["local_date"] == "2099-12-31":
                    ...
                    logger.error(
                        f"  🚨 VULNERABILITY: User {user['index']} updated immutable completion!"
                    )
                else:
            ...
            try:
                ...
                check = (
                    self.supabase.table("subtask_completions")
                    .select("id")
                    .eq("id", completion_id)
                    .execute()
                )
                if not check.data:
                    ...
                    logger.error(
                        f"  🚨 VULNERABILITY: User {user['index']} deleted immutable completion!"
                    )
                else:
        ...
        logger.info(
            f"  Immutability violations: {self.attack_attempts['immutability_violations']} attempts"
        )
            ...
            try:
                ...
                if response.data:
                    ...
                    if len(unique_users) > 1:
                        ...
                        logger.error(
                            f"  🚨 VULNERABILITY: Batch query leaked {len(unique_users)} users' data from {table}!"
                        )
    ...
    def cleanup_test_data(self):
        ...
        try:
            ...
            for user in self.test_users:
                # Delete user's data
                self.supabase.table("ai_artifacts").delete().eq(
                    "user_id", user["profile_id"]
                ).execute()
                self.supabase.table("ai_runs").delete().eq("user_id", user["profile_id"]).execute()
                self.supabase.table("triad_tasks").delete().eq(
                    "user_id", user["profile_id"]
                ).execute()
                self.supabase.table("daily_aggregates").delete().eq(
                    "user_id", user["profile_id"]
                ).execute()
                ...
                self.supabase.table("subtask_instances").delete().eq(
                    "user_id", user["profile_id"]
                ).execute()
                self.supabase.table("subtask_templates").delete().eq(
                    "user_id", user["profile_id"]
                ).execute()
                self.supabase.table("journal_entries").delete().eq(
                    "user_id", user["profile_id"]
                ).execute()
                self.supabase.table("goals").delete().eq("user_id", user["profile_id"]).execute()
                self.supabase.table("identity_docs").delete().eq(
                    "user_id", user["profile_id"]
                ).execute()
    ...
    def print_summary(self):
        ...
        logger.info(
            f"  • Immutability violations: {self.attack_attempts['immutability_violations']}"
        )

        total_attempts = (
            sum(self.attack_attempts.values()) - self.attack_attempts["successful_attacks"]
        )
