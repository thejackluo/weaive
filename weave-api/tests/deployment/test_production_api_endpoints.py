...
class TestAuthenticatedEndpoints:
    ...
    def test_production_goals_endpoint_with_auth(
        self, production_api_url, production_jwt_secret, http_session
    ):
        ...
        token = create_test_jwt_for_production(
            user_id=test_user_id, email=test_email, jwt_secret=production_jwt_secret
        )

        headers = {"Authorization": f"Bearer {token}"}
        response = http_session.get(f"{production_api_url}/api/goals", headers=headers, timeout=10)
    ...
    def test_production_completions_endpoint_with_auth(
        ...
        payload = {"subtask_instance_id": "test-subtask-id", "completed_at": "2025-12-23T10:00:00Z"}

        response = http_session.post(
            f"{production_api_url}/api/completions", headers=headers, json=payload, timeout=10
        )
    ...
    def test_production_journal_endpoint_with_auth(
        ...
        response = http_session.post(
            f"{production_api_url}/api/journal-entries", headers=headers, json=payload, timeout=10
        )
    ...
    def test_production_transcribe_endpoint_with_auth(
        ...
        response = http_session.post(
            f"{production_api_url}/api/transcribe", headers=headers, timeout=10
        )
...
class TestErrorHandling:
    ...
    def test_production_endpoints_reject_invalid_auth(self, production_api_url, http_session):
        """Verify endpoints return 401 Unauthorized with invalid JWT."""
        invalid_token = "invalid.jwt.token"
        headers = {"Authorization": f"Bearer {invalid_token}"}

        # Test multiple endpoints
        endpoints = ["/api/goals", "/api/journal-entries", "/api/completions"]

        for endpoint in endpoints:
            if endpoint == "/api/completions":
                response = http_session.post(
                    f"{production_api_url}{endpoint}", headers=headers, json={}, timeout=10
                )
            else:
                response = http_session.get(
                    f"{production_api_url}{endpoint}", headers=headers, timeout=10
                )
    ...
    def test_production_endpoints_reject_wrong_signature(self, production_api_url, http_session):
