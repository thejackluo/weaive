...
skip_bedrock = pytest.mark.skipif(
    not AWS_ACCESS_KEY or not AWS_SECRET_KEY, reason="AWS credentials not configured in .env"
)
...
def test_bedrock_real_api():
    ...
    model = (
        "us.anthropic.claude-3-5-haiku-20241022-v1:0"  # Inference profile ID: $0.25/$1.25 per MTok
    )
...
def test_fallback_chain_real_apis():
    ...
    response = deterministic_provider.complete(
        "test", module="triad", variant="default", task_1="Task A", task_2="Task B", task_3="Task C"
    )
...
def test_cost_tracking_accuracy():
    ...
    print("\n🧮 Manual verification:")
    print(
        f"   Input cost: {response.input_tokens} tokens × $0.15/MTok = ${expected_input_cost:.6f}"
    )
    print(
        f"   Output cost: {response.output_tokens} tokens × $0.60/MTok = ${expected_output_cost:.6f}"
    )
...
def test_deterministic_provider_free():
    ...
    modules = [
        ...
        (
            "weekly_insights",
            {"pattern_1": "Consistency", "pattern_2": "Focus", "focus_area": "Deep work"},
        ),
    ]
