...
class TestDataExportEndpoint:
    ...
    def test_data_export_returns_complete_user_data(
        ...
        assert len(export_data["subtasks"]) >= 9, f"Expected at least 9 subtasks, got {len(export_data['subtasks'])}"
        ...
        assert len(export_data["completions"]) == 10, f"Expected 10 completions, got {len(export_data['completions'])}"
        ...
        assert len(export_data["journal_entries"]) == 5, f"Expected 5 journal entries, got {len(export_data['journal_entries'])}"
        ...
        assert len(export_data["ai_chat_history"]) == 3, f"Expected 3 chat messages, got {len(export_data['ai_chat_history'])}"
        ...
        assert len(export_data["proof_photos"]) == 3, f"Expected 3 proof photos, got {len(export_data['proof_photos'])}"
    ...
    def test_data_export_includes_signed_storage_urls(
        ...
        assert "supabase.co" in first_photo or "localhost" in first_photo, "Not a Supabase Storage URL"
        assert "sign" in first_photo or "token" in first_photo, "URL is not signed (missing signature)"
...
class TestAccountDeletionEndpoint:
    ...
    def test_account_deletion_soft_deletes_user(
        ...
        assert "30 days" in data["data"]["message"], "Response should mention 30-day recovery period"

        # Verify user_profiles.deleted_at is set
        user_profile = supabase_client.table("user_profiles").select("*").eq("id", user["user_id"]).execute()
        ...
        deleted_at = datetime.fromisoformat(user_profile.data[0]["deleted_at"].replace("Z", "+00:00"))
    ...
    def test_account_recovery_within_30_days(
        ...
        supabase_client.table("user_profiles").update({
            "deleted_at": five_days_ago.isoformat()
        ...
        user_profile = supabase_client.table("user_profiles").select("deleted_at").eq("id", user["user_id"]).execute()
    ...
    def test_account_hard_delete_after_30_days(
        ...
        completions = supabase_client.table("subtask_completions").select("*").eq("user_id", user["user_id"]).execute()
        assert len(completions.data) == 0, "Completions should be cascade deleted"

        journal_entries = supabase_client.table("journal_entries").select("*").eq("user_id", user["user_id"]).execute()
    ...
    def test_account_deletion_cascades_across_all_tables(
        ...
        for table in tables_to_verify:
            result = supabase_client.table(table).select("count", count="exact").eq("user_id", user_id).execute()
        ...
        for table in tables_to_verify:
            result = supabase_client.table(table).select("count", count="exact").eq("user_id", user_id).execute()
            assert result.count == 0, f"Table {table} should have 0 records after cascade delete, but has {result.count}"
