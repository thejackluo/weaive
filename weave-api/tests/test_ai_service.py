        ...
        data=[
            {
                ...
                "output_tokens": 50,
                "model": "gpt-4o-mini",
                "ai_artifacts": [{"json": {"content": "Cached response"}}],
            }
        ]
    )

    response = service.generate(
        user_id="user_123", user_role="user", user_tier="free", module="triad", prompt="Test prompt"
    )
...
def test_ai_service_rate_limit_blocks_request(mock_db, ai_service):
    ...
    with patch.object(
        ai_service.rate_limiter,
        "check_user_limit",
        side_effect=RateLimitError(
            "Rate limit exceeded", user_id="user_123", user_tier="free", limit="10 calls/day"
        ),
    ):
...
def test_ai_service_fallback_chain(mock_db):
    ...
    with patch.object(
        BedrockProvider, "complete", side_effect=AIProviderError("Bedrock error", "bedrock")
    ):
        with patch.object(
            OpenAIProvider, "complete", side_effect=AIProviderError("OpenAI error", "openai")
        ):
            with patch.object(
                AnthropicProvider,
                "complete",
                side_effect=AIProviderError("Anthropic error", "anthropic"),
            ):
                # Mock rate limiter (pass)
