    ...
    try:
        ...
        try:
            binds_response = (
                ...
                .select(
                    "id, goal_id, title, description, frequency_type, frequency_value, is_archived, created_at, updated_at"
                )
                .eq("goal_id", goal_id)
...
async def create_goal(
    ...
    try:
        ...
        if goal_data.qgoals:
            logger.info(
                f"⚠️ Skipping Q-goals creation (table not yet implemented) - {len(goal_data.qgoals)} qgoals provided"
            )
