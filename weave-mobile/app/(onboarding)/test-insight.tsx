  ...
  return (
    <SafeAreaView className="flex-1 bg-white">
      <ScrollView className="flex-1 p-4">
        <View className="mb-6">
          <Text className="text-2xl font-bold text-neutral-900 mb-2">Story 1.3 Test Screen</RNText>
        ...
        {testScenarios.map((scenario, index) => (
          ...
          >
            <Text className="text-base font-semibold text-blue-900 mb-1">{scenario.title}</RNText>
            <Text className="text-sm text-blue-700 mb-2">
              Painpoints:{' '}
              {scenario.painpoints.length === 0 ? 'None' : scenario.painpoints.join(', ')}
            </RNText>
            <Text className="text-xs text-neutral-600">{scenario.description}</RNText>
          </Pressable>
        ))}

        <View className="mt-6 p-4 bg-neutral-100 rounded-xl">
          <Text className="text-sm font-semibold text-neutral-900 mb-2">Testing Checklist:</RNText>
          <Text className="text-xs text-neutral-700 mb-1">✓ Glass effect on cards</RNText>
          <Text className="text-xs text-neutral-700 mb-1">✓ Fade-in animation for first card</RNText>
          <Text className="text-xs text-neutral-700 mb-1">
            ✓ Slide-up animation for second card (200ms delay)
          </RNText>
          <Text className="text-xs text-neutral-700 mb-1">✓ CTA button appears after cards</RNText>
          <Text className="text-xs text-neutral-700 mb-1">
