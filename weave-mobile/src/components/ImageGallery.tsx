    ...
    >
      ...
      {item.ai_quality_score && item.ai_quality_score >= 4 && (
        <View className="absolute bottom-2 right-2 bg-black/60 rounded px-2 py-1">
          <Text className="text-white text-xs">⭐ {item.ai_quality_score}/5</RNText>
        </View>
      )}
    </TouchableOpacity>
  );
  ...
  if (isLoading) {
    return (
      <View className="flex-1 items-center justify-center bg-neutral-900">
        ...
        <Text className="text-neutral-400 mt-4">Loading images...</RNText>
      </View>
    );
  }
  ...
  if (captures.length === 0) {
    return (
      <View className="flex-1 items-center justify-center bg-neutral-900 p-6">
        ...
        <Text className="text-neutral-400 text-lg mt-4 text-center">No images yet</RNText>
  ...
  const renderFooter = () => {
    ...
    return (
      <View className="py-4">
        ...
        <Text className="text-neutral-400 text-center mt-2 text-xs">Loading more...</RNText>
      </View>
    );
  };
